...
const apiSet = require('./api.erc20')
...
const testName = 'erc20'
const testContractName = 'ERC20M'
const deployConstructorArguments = ['ChainzToken', 'CZT']

// Environment variables
const args = process.argv.slice(2)

const { setting, httpRpcUrl, configObj } = utils.prepareTestcase(testName, args)
let testContractAddr = configObj.erc20Address
...
const prepareOld = async (req, res) => {
  let output = support.getDefaultResponseObj(testName, testContractAddr)
  output = await apiSet.prepareDeposit(test, setting.accountConf, configObj.ownerPrivKey, prepareAmount, output)
  latestTxid = output.latestTxid
  utils.responseJson(res, output)
}
...
const getBalanceOf = async (req, res) => {
  ...
  try {
    ...
    let responseData = await test.balanceOf(params.account)
    output.account = params.account
    output.result = true
    output.balanceOf = responseData
  } catch (err) {
...
const getInitialSupply = async (req, res) => {
  ...
  try {
    ...
    output.result = true
    output.initialSupply = responseData
  } catch (err) {
    //ERROR(`Error occurred: ${err}`)
    output.message = err
  }
...
router.route('/getBalanceOf/:account').get(getBalanceOf)
router.route('/getDecimals').get(getDecimals)
router.route('/getInitialSupply').get(getInitialSupply)
